...
export default {
  ...
  async fetch(context) {
    ...
    if(context.store.state.caseStudies.loaded !== '1') {
      const caseSudiesRefRes = await context.app.$storyapi.get(`cdn/stories/`, { starts_with: 'case-studies/', version: 'draft' })
      context.store.commit('caseStudies/setCaseStudies', caseSudiesRefRes.data.stories)
